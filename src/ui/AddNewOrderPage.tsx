...
import AddNewOrderStyles from './styles/AddNewOrderStyles';
...
type AddNewOrderProps = NativeStackScreenProps<RootStackParamList, 'AddNewOrder'>;


export default function AddNewOrderPage({ navigation, route }: AddNewOrderProps) {
    ...
    return (
        <View style={AddNewOrderStyles.container}>
            <AppBar title="Add New Order" showIconLeft iconLeft="keyboard-backspace" iconLeftColor="black" onPressLeft={() => navigation.pop()} />
...
function OrderFormComponent({ listProductChoosed, listOfProduct, visible, onRequestClose, onPressForOptions }: { listProductChoosed: Array<any>, listOfProduct: Array<any>, visible: boolean, onRequestClose: () => void, onPressForOptions: () => void }) {
    return (
        <View style={AddNewOrderStyles.content}>
            <ScrollView>
                <View style={{ width: '100%' }}>
                    <View style={AddNewOrderStyles.labelWithRequired}>
                        <Text style={AddNewOrderStyles.label}>Customer Name</Text>
                        <Spacer width={5} />
                        <Text style={AddNewOrderStyles.required}>*</Text>
                    </View>
                    <Spacer height={5} />
                    <TextInput style={AddNewOrderStyles.textInput} placeholder="Input customer name" placeholderTextColor={grey} />
                </View>
                <Spacer height={10} />
                <HorizontalLine />
                <Spacer height={10} />
                <Text style={AddNewOrderStyles.section}>Product Detail</Text>
                <Spacer height={10} />
                {listProductChoosed.map((e, i) => <View style={{ width: '100%', marginTop: i > 0 ? 10 : 0 }} key={e.id}>
                    <View style={{ width: '100%' }}>
                        <View style={AddNewOrderStyles.labelWithRequired}>
                            <Text style={AddNewOrderStyles.label}>Product Name</Text>
                            <Spacer width={5} />
                            <Text style={AddNewOrderStyles.required}>*</Text>
                        </View>
                        <Spacer height={5} />
                        <TouchableOpacity onPress={onPressForOptions}>
                            <TextInput style={AddNewOrderStyles.textInput} placeholder="Select product name" placeholderTextColor={grey} editable={false} selectTextOnFocus={false} />
                        </TouchableOpacity>
                    </View>
                    <View style={{ width: '100%' }}>
                        <Text style={AddNewOrderStyles.label}>Price</Text>
                        <Spacer height={5} />
                        <TextInput style={AddNewOrderStyles.textInputDisable} placeholder="Price" placeholderTextColor={grey} />
                    </View>
                    <View style={{ width: '100%' }}>
                        <View style={AddNewOrderStyles.labelWithRequired}>
                            <Text style={AddNewOrderStyles.label}>Quantity</Text>
                            <Spacer width={5} />
                            <Text style={AddNewOrderStyles.required}>*</Text>
                        </View>
                        <Spacer height={5} />
                        <TextInput style={AddNewOrderStyles.textInput} placeholder="Input quantity" placeholderTextColor={grey} />
                    </View>
                </View>)}
                <Spacer height={10} />
                <TouchableOpacity style={{ backgroundColor: darkButton, width: 184, borderColor: darkButton, borderWidth: 1, borderRadius: 4, height: 36, alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ color: 'white', fontSize: 15, fontWeight: '700' }}>Add More Product</Text>
                </TouchableOpacity>
                <Spacer height={10} />
                <HorizontalLine />
                <Spacer height={10} />
                <View style={{ width: '100%' }}>
                    <Text style={AddNewOrderStyles.label}>Total price</Text>
                    <Spacer height={5} />
                    <TextInput style={AddNewOrderStyles.textInputDisable} placeholder="Total price" placeholderTextColor={grey} />
                </View>
            </ScrollView>
            <Modal
                animationType="slide"
